...
senha_forte_regexp = re.compile(
    r'^' #com positive lookheads
    r'(?=.*[a-z])' # pelo menos uma letra minúscula 
    r'(?=.*[A-Z])' # pelo menos uma letra maíuscula
    r'(?=.*[0-9])' # pleo menos um número
    r'(?=.*[ -\/:-@[-`{-~])' # pelo menos um caractere especial desses
    r'.{12,}' #12 caracteres totais
    r'$',
    flags=re.M
)
